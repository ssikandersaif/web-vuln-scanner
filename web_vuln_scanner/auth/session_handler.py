...
def set_dvwa_security(session, base_url, level="low"):
    ...
    return True
def login_juice_shop(session, base_url, email, password):
    """
    Login to OWASP Juice Shop.
    
    Args:
        session: The requests session
        base_url: Juice Shop URL (e.g., http://localhost:3000)
        email: User email
    ...
    # Juice Shop expects JSON, not form data
    login_data = {
        "email": email,
        "password": password
    }
    
    # Send as JSON (note: json= not data=)
    ...
    if response.status_code == 200:
        ...
        print(f"[+] Logged into Juice Shop as '{email}'")
        return True
    else:
        print("[!] Juice Shop login failed")
        return False
